...
-- Show products purchased together if any.

SELECT ord.orderDate, ord.orderNumber, ordt.quantityOrdered, pd.buyprice, pay.amount, pay.paymentDate,
SUM(pay.amount)  OVER(PARTITION BY orderNumber) AS sum_amount
FROM orders ord
JOIN customers cust
ON ord.customerNumber = cust. customerNumber
JOIN orderdetails ordt
